...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const modalWhatsappLink = document.getElementById('modal-whatsapp-link'); // Novo: Link do WhatsApp no modal
            ...
            if (!input.value.trim()) {
                ...
                if (input.id === 'email') {
                    if (!/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/.test(input.value)) {
                        ...
                        const errorDiv = document.createElement('span');
                        errorDiv.classList.add('error-message');
                        errorDiv.textContent = 'E-mail inválido. Por favor, verifique o formato (ex: seuemail@dominio.com).';
                        input.parentNode.insertBefore(errorDiv, input.nextSibling);
                    ...
                    // Validação básica de CPF: 11 dígitos e não todos iguais (para evitar CPFs como 111.111.111-11)
                    if (unmaskedCPF.length !== 11 || /^(\d)\1{10}$/.test(unmaskedCPF)) {
                        isValid = false;
                        input.classList.add('input-error');
                        const errorDiv = document.createElement('span');
                        errorDiv.classList.add('error-message');
                        errorDiv.textContent = 'CPF inválido. Necessita 11 dígitos válidos.';
                        input.parentNode.insertBefore(errorDiv, input.nextSibling);
                        console.log(`Validação FALHOU para ${input.id}: CPF com menos de 11 dígitos ou inválido: ${unmaskedCPF.length}`);
                    } else {
                        console.log(`Validação OK para ${input.id}: ${input.value}`);
                    }
                    ...
                    // Adicionei uma verificação para o 9 inicial em números de celular com 11 dígitos
                    if (unmaskedPhone.length < 10 || unmaskedPhone.length > 11) {
                        isValid = false;
                        input.classList.add('input-error');
                        const errorDiv = document.createElement('span');
                        errorDiv.classList.add('error-message');
                        errorDiv.textContent = 'Telefone inválido. Necessita 10 (fixo) ou 11 (celular) dígitos, incluindo o DDD.';
                    } else if (unmaskedPhone.length === 11 && unmaskedPhone.charAt(2) !== '9') {
                        // Se tem 11 dígitos, o 3º dígito (índice 2) deve ser '9' para ser um celular válido no Brasil.
                        // (Considerando que os 2 primeiros são o DDD)
                        isValid = false;
                        input.classList.add('input-error');
                        const errorDiv = document.createElement('span');
                        errorDiv.classList.add('error-message');
                        errorDiv.textContent = 'Telefone celular inválido. Números de 11 dígitos devem começar com 9 após o DDD.';
                        input.parentNode.insertBefore(errorDiv, input.nextSibling);
                    }
                    else {
                        console.log(`Validação OK para ${input.id}: ${input.value}`);
    ...
    document.querySelectorAll('[data-next-step]').forEach(button => {
        button.addEventListener('click', (e) => {
            ...
            // Se o botão for da etapa de dados, previna o comportamento padrão de submit
            if (currentStepId === 'step-dados') {
                e.preventDefault(); // CHAVE PARA RESOLVER O PROBLEMA!
            }
    ...
    document.querySelectorAll('[data-prev-step]').forEach(button => {
        button.addEventListener('click', (e) => {
            e.preventDefault(); // Previne o padrão para botões "Voltar" (se forem tipo submit)
    ...
    // Lógica para "finalizar" a simulação (agora com modal e dados para WhatsApp)
    document.getElementById('finalize-loan-btn').addEventListener('click', (e) => {
        e.preventDefault(); // Previne o submit do formulário quando finalizar
        console.log('Botão "Finalizar Empréstimo" clicado.');
        
        // Coleta todos os dados do formulário
        const amount = document.getElementById('loan-amount-display').textContent;
        const installments = document.getElementById('installments-display').textContent;
        ...
        const whatsappMessage = `Olá, gostaria de finalizar minha simulação de empréstimo do CréditoAgora.
...
CPF: ${cpf}`;
        ...
        // ATENÇÃO: SUBSTITUA '5588999999999' pelo seu número real do WhatsApp (PaísDDDNúmero)
        const whatsappNumber = '5588999999999'; 
        const whatsappLink = `https://wa.me/${whatsappNumber}?text=${encodedMessage}`;
        
        // Atribui o link ao botão do WhatsApp no modal
        if (modalWhatsappLink) {
            modalWhatsappLink.href = whatsappLink;
        }

        console.log('Dados da Simulação Finalizada:', { amount, installments, fullName, email, phone, cpf });
